...
\begin{aligned}
y(n)=&
-a_1y(n-1)-a_2y(n-2)-\cdots-a_Ny(n-N)\\
...
\begin{aligned}
Y(z)=&
-a_1z^{-1}Y(z)-\cdots-a_Nz^{-N}Y(z)\\
&+b_0X(z)+b_1z^{-1}X(z)+\cdots+b_Mz^{-M}X(z)
\end{aligned}
$$
...
$$
Y(z)+2z^{-1}Y(z)+3z^{-2}Y(z)
=
4X(z)+5z^{-1}X(z)+6z^{-2}X(z)
$$
...
$$
H_i(z)=
\frac{C_{0i}+C_{1i}z^{-1}+C_{2i}z^{-2}}
{d_{0i}+d_{1i}z^{-1}+d_{2i}z^{-2}}
$$
...
H(z)=
\frac{(1+z^{-1})(1+3z^{-1})}
{(1+\frac12z^{-1})(1+\frac13z^{-1})(1+\frac14z^{-1})}
}
$$
...
-\frac56z^{-2}Y(z)
+\frac16z^{-3}Y(z)\\
&+3X(z)-5z^{-1}X(z)+9z^{-2}X(z)-3z^{-3}X(z)
\end{aligned}
}
$$
...
\boxed{
Y(z)=
3W(z)-5z^{-1}W(z)+9z^{-2}W(z)-3z^{-3}W(z)
}
$$
...
\frac{1+\frac13z^{-1}}
{
\left(1-\frac12z^{-1}+\frac13z^{-2}\right)
\left(1-\frac13z^{-1}+\frac12z^{-2}\right)
}
$$
...
\boxed{
H(z)=
\frac{0.146}{1-0.414z^{-1}}
+
\frac{0.853}{1+2.414z^{-1}}
}
$$
...
$$
\boxed{
\text{Linear-phase symmetry reduces the number of multipliers.}
}
$$
...
h(n)=
\left\{
\frac13,\frac15,\frac23,\frac15,\frac13
\right\}
}
$$
...
$$
W(z)=
X(z)-a_1z^{-1}W(z)-\cdots-a_Nz^{-N}W(z)
$$
...
$$
Y(z)=
b_0W(z)+b_1z^{-1}W(z)+\cdots+b_Mz^{-M}W(z)
$$
...
\begin{aligned}
y(n)=&
-0.5y(n-1)+0.25y(n-2)+0.125y(n-3)\\
&+x(n)+0.5x(n-1)+0.75x(n-2)
\end{aligned}
$$
...
\begin{aligned}
y(n)=&
-\frac38y(n-1)+\frac3{32}y(n-2)+\frac1{64}y(n-3)\\
&+x(n)+3x(n-1)+2x(n-2)
\end{aligned}
$$
...
$$
\boxed{
\text{Conversion of a difference equation or transfer function into a network.}
}
$$
...
$$
\boxed{
\text{Adder + Constant multiplier + Unit delay}
}
$$
...
\boxed{
\begin{array}{ll}
\text{Direct-I} &\rightarrow \text{separate delay lines}\\
\text{Direct-II} &\rightarrow \text{shared delay line}\\
\text{Transposed} &\rightarrow \text{reverse + interchange}\\
\text{Cascade} &\rightarrow \text{factor/product}\\
\text{Parallel} &\rightarrow \text{partial fractions/sum}\\
\text{Lattice} &\rightarrow \text{reflection coefficients}\\
\text{Ladder} &\rightarrow \text{sequential division}\\
\text{Linear phase} &\rightarrow \text{symmetry + fewer multipliers}
\end{array}
}
$$
