...
$$
\boxed{ \text{discrete one-dimensional signals} }
$$

and

$$
\boxed{ \text{single-input, single-output discrete-time systems}. }
$$
...
$$
\boxed{ x(n)\longrightarrow \boxed{\text{SYSTEM}} \longrightarrow y(n) }
$$
...
$$
\boxed{ \text{more bits}\Rightarrow\text{higher possible precision} }
$$
...
$$
\boxed{ \begin{array}{ll} 1.&\text{Graphical representation}\\ 2.&\text{Functional representation}\\ 3.&\text{Tabular representation}\\ 4.&\text{Sequence representation} \end{array} }
$$
...
$$
x(-2)=3,\quad x(-1)=2,\quad x(0)=0,\quad x(1)=3,\quad x(2)=1,\quad x(3)=2.
$$
...
$$
x(n)= \begin{cases} 2^n, & n\ge0,\\ 0, & n<0. \end{cases}
$$

A piecewise function may also be used:

$$
x(n)= \begin{cases} 3, & n=-2,\\ 2, & n=-1,\\ 0, & n=0,\\ 3, & n=1,\\ 1, & n=2,\\ 2, & n=3. \end{cases}
$$
...
$$
\boxed{ u(n)= \begin{cases} 1,&n\ge0,\\ 0,&n<0. \end{cases} }
$$
...
$$
\boxed{ u(n-k)= \begin{cases} 1,&n\ge k,\\ 0,&n<k. \end{cases} }
$$
...
$$
\boxed{ u(n-k)\Rightarrow\text{shift right by }k }
$$
...
$$
\boxed{ r(n)= \begin{cases} n,&n\ge0,\\ 0,&n<0. \end{cases} }
$$
...
$$
\boxed{ p(n)= \begin{cases} \dfrac{n^2}{2},&n\ge0,\\ 0,&n<0. \end{cases} }
$$

Therefore,

$$
\boxed{ p(n)=\frac{n^2}{2}u(n). }
$$

The shifted form is

$$
\boxed{ p(n-k)= \frac{(n-k)^2}{2}u(n-k). }
$$
...
$$
\boxed{ \delta(n-k)= \begin{cases} 1,&n=k,\\ 0,&n\ne k. \end{cases} }
$$

Therefore:

$$
\boxed{ \delta(n-k)\text{ occurs at }n=k. }
$$
...
$$
\boxed{ x(n)=a^n e^{j(\omega_0n+\phi)}. }
$$
...
$$
\boxed{ x(n) = a^n\cos(\omega_0n+\phi) + j\,a^n\sin(\omega_0n+\phi). }
$$
...
$$
\sum_{n=-\infty}^{\infty} e^{3n}\delta(n-3)
$$

and similar expressions involving

$$
\delta(n-k),\qquad \cos(3n),\qquad n^2.
$$

Use the sifting property:

$$
\boxed{ \sum_{n=-\infty}^{\infty} f(n)\delta(n-k) = f(k). }
$$

Therefore,

$$
\sum_{n=-\infty}^{\infty} e^{3n}\delta(n-3) = e^9.
$$

Similarly,

$$
\sum_{n=-\infty}^{\infty} \delta(n-2)\cos(3n) = \boxed{\cos6}.
$$
...
$$
\boxed{ \begin{array}{ll} 1.&\text{Time shifting}\\ 2.&\text{Time reversal}\\ 3.&\text{Time scaling}\\ 4.&\text{Amplitude scaling}\\ 5.&\text{Signal addition}\\ 6.&\text{Signal multiplication} \end{array} }
$$
...
$$
\boxed{ x(n-k)\Rightarrow\text{right shift} }
$$
...
$$
x(n)= \begin{cases} 1,&-2\le n\le3,\\ 0,&\text{otherwise}. \end{cases}
$$

### (b)

$$
\boxed{ x(n)=u(n+4)-u(n-2) }
$$
...
$$
\boxed{ x_1(n)+x_2(n) = \{3,5,7,2,3\}. }
$$

The subtraction is

$$
\boxed{ x_1(n)-x_2(n) = \{-1,-1,-1,0,7\}. }
$$
...
$$
\boxed{ x(n)= \delta(n+2)+\delta(n+1)+\delta(n)+\delta(n-1). }
$$
...
$$
\boxed{ x(n)= \delta(n-2)+\delta(n-3)+\delta(n-4)+\delta(n-5) }
$$
...
$$
\boxed{ \text{deterministic}\Rightarrow\text{no uncertainty} }
$$

$$
\boxed{ \text{random}\Rightarrow\text{uncertainty} }
$$
...
$$
A\sin[\omega_0(n+N)+\phi] = A\sin(\omega_0n+\phi).
$$
...
$$
\boxed{ \frac{\omega_0}{2\pi} \text{ must be rational}. }
$$
...
$$
\frac{4}{2\pi} = \frac2\pi.
$$

Since this is irrational,

$$
\boxed{ x(n)=\cos(4n) \text{ is non-periodic}. }
$$
...
$$
\boxed{ N_0=\operatorname{LCM}(3,5)=15. }
$$
...
$$
\boxed{ N_0=\operatorname{LCM}(N_1,N_2,\ldots) }
$$
...
$$
\boxed{ E= \sum_{n=-\infty}^{\infty} |x(n)|^2. }
$$

For a real signal,

$$
\boxed{ E= \sum_{n=-\infty}^{\infty} x^2(n). }
$$
...
$$
\boxed{ P= \lim_{N\to\infty} \frac1{2N+1} \sum_{n=-N}^{N} |x(n)|^2. }
$$
...
$$
\boxed{ \text{Energy signal}\Rightarrow P=0 }
$$

$$
\boxed{ \text{Power signal}\Rightarrow E=\infty }
$$

Signals such as

$$
u(n),\qquad nu(n),\qquad n^2u(n)
$$
...
$$
\sum_{n=0}^{\infty} \left(\frac12\right)^{2n}
$$
...
$$
\boxed{ x(n)=x_e(n)+x_o(n). }
$$

The even component is

$$
\boxed{ x_e(n)= \frac12[x(n)+x(-n)]. }
$$

The odd component is

$$
\boxed{ x_o(n)= \frac12[x(n)-x(-n)]. }
$$
...
$$
\boxed{ x_e(n)=\frac{x(n)+x(-n)}2 }
$$

and

$$
\boxed{ x_o(n)=\frac{x(n)-x(-n)}2. }
$$
...
$$
\boxed{ \text{past/future sample present} \Rightarrow \text{dynamic} }
$$
...
$$
\boxed{ y(n)=F[x(n),x(n-1),x(n-2),\ldots] }
$$
...
$$
\boxed{ ax_1(n)+bx_2(n) \rightarrow ay_1(n)+by_2(n) }
$$
...
$$
a^2x_1^2(n) + 2abx_1(n)x_2(n) + b^2x_2^2(n).
$$
...
$$
\boxed{ y(n)=x^2(n)\text{ is nonlinear}. }
$$
...
$$
T[x(n-k)] = \frac12[x(n-k)+x(-n-k)]
$$

whereas

$$
y(n-k) = \frac12[x(n-k)+x(-n+k)].
$$
...
$$
\boxed{ |x(n)|<\infty \Rightarrow |y(n)|<\infty. }
$$
...
$$
\boxed{ \text{Absolutely summable }h(n) \Rightarrow \text{BIBO stable}. }
$$
...
$$
h(n)= \delta(n) +\frac12\delta(n-1) +\frac14\delta(n-2).
$$

Therefore,

$$
\sum|h(n)| = 1+\frac12+\frac14 = \frac74<\infty.
$$
...
$$
\boxed{ \text{find }h(n) \rightarrow \sum|h(n)| \rightarrow \text{finite?} }
$$
...
$$
\boxed{ \text{linear + stable + non-causal + time-varying} }
$$
...
$$
\boxed{ \text{dynamic, linear, causal, shift-invariant, stable}. }
$$
...
$$
\boxed{ \text{causal LTI} \iff h(n)=0\text{ for }n<0 }
$$

and

$$
\boxed{ \text{stable LTI} \iff \sum|h(n)|<\infty. }
$$
...
$$
y(n) = \sum_k b_kx(n-k) - \sum_r a_ry(n-r),
$$
...
$$
\boxed{ \text{present/past terms only}\Rightarrow\text{causal} }
$$
...
$$
\boxed{ \begin{array}{c} \text{Static/Dynamic}\\ \downarrow\\ \text{Linear/Nonlinear}\\ \downarrow\\ \text{Causal/Noncausal}\\ \downarrow\\ \text{Time-invariant/Time-varying}\\ \downarrow\\ \text{Stable/Unstable} \end{array} }
$$
...
$$
\boxed{ x(n) = 3\delta(n) +5\delta(n-1) +2\delta(n-2) +\delta(n-3) +4\delta(n-4) +7\delta(n-5). }
$$
...
$$
x(n_1)=A_1,\quad x(n_2)=A_2,\quad \ldots
$$

Then write

$$
\boxed{ x(n)= A_1\delta(n-n_1) + A_2\delta(n-n_2) +\cdots }
$$
...
$$
\boxed{ T[ax_1+bx_2] \stackrel{?}{=} aT[x_1]+bT[x_2]. }
$$
...
$$
\boxed{ \text{stable poles lie inside the unit circle}. }
$$
...
$$
\boxed{ \omega_0/(2\pi)\in\mathbb{Q} \Rightarrow\text{periodic} }
$$
...
$$
\boxed{ \omega_0/(2\pi)\text{ must be rational}. }
$$
...
$$
\sin(\omega n),\quad \cos(\omega n),\quad e^{j\omega n}.
$$
...
$$
\boxed{ T[ax_1+bx_2] = aT[x_1]+bT[x_2] }
$$
...
$$
\boxed{ x(n-k)=\text{delay} }
$$

$$
\boxed{ x(-n)=\text{time reversal} }
$$

$$
\boxed{ x_e=\frac{x+x(-n)}2 }
$$

$$
\boxed{ x_o=\frac{x-x(-n)}2 }
$$
...
$$
\boxed{ \omega_0/(2\pi)\text{ rational} }
$$
...
$$
\boxed{ P=\lim_{N\to\infty} \frac1{2N+1}\sum_{-N}^{N}|x(n)|^2 }
$$

$$
\boxed{ \text{causal signal}:x(n)=0,\ n<0 }
$$

$$
\boxed{ \text{linear}:T[ax_1+bx_2]=aT[x_1]+bT[x_2] }
$$

$$
\boxed{ \text{time invariant}:T[x(n-k)]=y(n-k) }
$$

$$
\boxed{ \text{stable LTI}:\sum|h(n)|<\infty }
$$

$$
\boxed{ \text{FIR}=\text{finite }h(n) }
$$

$$
\boxed{ \text{IIR}=\text{infinite }h(n) }
$$
...
$$
\boxed{ \text{Signal} \rightarrow \text{Representation} \rightarrow \text{Elementary Signals} \rightarrow \text{Operations} \rightarrow \text{Signal Classification} \rightarrow \text{System Classification} \rightarrow \text{Arbitrary Sequence} }
$$
