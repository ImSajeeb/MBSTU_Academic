...
\begin{aligned}
y(n)
&=\sum_{k=0}^{n}3^k\left(\frac13\right)^{n-k}\\
&=3^{-n}\sum_{k=0}^{n}9^k.
\end{aligned}
\]
...
\[
x(n)=\cos\left(\frac n2\right)u(n),
\qquad
...
\[
\cos\left(\frac k2\right)
=
\operatorname{Re}\left\{e^{jk/2}\right\}.
\]
...
\[
\boxed{
y(n)=2\left[1-\left(\frac12\right)^{n+1}\right],
\qquad 0\le n\le9
}
\]
...
s(n)=
\frac43
\left[1-\left(\frac14\right)^{n+1}\right]u(n)
}
\]
...
\[
X(z)=x_0+x_1z+x_2z^2+\cdots,\qquad
H(z)=h_0+h_1z+h_2z^2+\cdots,
\]
...
\[
\boxed{
\mathbf y=\mathbf H_{\rm circ}\mathbf x
}
\]
...
\[
\boxed{
\{5.2,\ 6,\ 6.48,\ 6.64,\ 6.48,\ 6,\ 5.2,\ 4.08\}
}
\]
...
\[
y(n)=
\sum_{k=0}^{3}x_1(k)x_2[(n-k)\bmod4].
\]
...
=
\text{convolution with one sequence folded}
}
\]
...
y=x*h
\rightarrow
\text{solve sequentially for unknown samples}
}
\]
...
\[
\boxed{
n_{y,\rm start}=n_{x,\rm start}+n_{h,\rm start}
}
\]
...
\[
\boxed{
R_{xx}(0)\text{ is the maximum autocorrelation value}
}
\]
...
Finding \(x(n)\) or \(h(n)\) when \(y(n)=x(n)*h(n)\) and the other sequence is known.
...
20. Compute \(x(n)*x(-n)\); the result must be even symmetric.
...
\boxed{
\begin{aligned}
&\text{1. Starting index correct}\\
&\text{2. Output length correct}\\
&\text{3. Arrow / }n=0\text{ location correct}\\
&\text{4. Flip/shift direction correct}\\
&\text{5. Every overlap multiplied correctly}\\
&\text{6. Circular calculations use modulo }N\\
&\text{7. Correlation includes a flip}\\
&\text{8. Final sequence has the correct number of samples}\\
&\text{9. Finite convolution passes the sum check}\\
...
\begin{aligned}
L_y&=L_x+L_h-1,\\[2mm]
n_{y,\mathrm{start}}&=n_{x,\mathrm{start}}+n_{h,\mathrm{start}},\\[2mm]
n_{y,\mathrm{end}}&=n_{x,\mathrm{end}}+n_{h,\mathrm{end}}.
\end{aligned}
}
\]
...
\boxed{
\begin{aligned}
\text{Linear convolution:}\quad
&y(n)=\sum_kx(k)h(n-k)\\[1mm]
\text{Finite length:}\quad
&L_y=L_x+L_h-1\\[1mm]
\text{Start index:}\quad
&n_{y,0}=n_{x,0}+n_{h,0}\\[1mm]
\text{Step response:}\quad
&s(n)=h(n)*u(n)\\[1mm]
\text{Circular convolution:}\quad
&y_p(n)=\sum_{k=0}^{N-1}x(k)h[(n-k)\bmod N]\\[1mm]
\text{Periodic extension:}\quad
&x_p(n)=\sum_{m=-\infty}^{\infty}x(n+mN)\\[1mm]
\text{Cross correlation:}\quad
&R_{xy}(n)=x(n)*y(-n)\\[1mm]
\text{Autocorrelation:}\quad
&R_{xx}(n)=x(n)*x(-n)\\[1mm]
\text{Correlation symmetry:}\quad
&R_{xy}(n)=R_{yx}(-n)\\[1mm]
\text{Autocorrelation symmetry:}\quad
&R_{xx}(n)=R_{xx}(-n)
\end{aligned}
}
\]
