...
$$
\boxed{ x^\ast (n) \ \xleftrightarrow{\text{DFS}}\ X^\ast (-k) }
$$
...
$$
\boxed{ x_e(n)= \frac12\left[x(n)+x^\ast (-n)\right] }
$$

$$
\boxed{ x_o(n)= \frac12\left[x(n)-x^\ast (-n)\right] }
$$
...
$$
\boxed{ \mathbf{x} = \frac1N \mathbf{W}_N^\ast  \mathbf{X}. }
$$

The textbook emphasizes the important relation

$$
\boxed{ \mathbf{W}_N^{-1} = \frac1N\mathbf{W}_N^\ast  }
$$
...
$$
\boxed{ \text{IDFT}\{X(k)\} = \frac1N \left[ \text{DFT}\{X^\ast (k)\} \right]^\ast  }
$$
...
Compute the DFT of $X^\ast (k)$, take its conjugate, and divide by $N=4$.
...
$$
\boxed{ x^\ast (n) \ \xleftrightarrow{\text{DFT}}\ X^\ast (N-k) }
$$
...
$$
\boxed{ \sum_{n=0}^{N-1} x_1(n)x_2^\ast (n) = \frac1N \sum_{k=0}^{N-1} X_1(k)X_2^\ast (k). }
$$
...
$$
\boxed{ r_{xy}(l) = \sum_{n=0}^{N-1} x(n)y^\ast ((n-l)\bmod N) }
$$

and

$$
\boxed{ \text{DFT}\{r_{xy}(l)\} = X(k)Y^\ast (k). }
$$
...
| circular correlation | $X(k)Y^\ast (k)$ |
| $\sum x(n)y^\ast (n)$ | $\frac1N\sum X(k)Y^\ast (k)$ |
...
$$
\boxed{ x^\ast (n) \leftrightarrow X^\ast (N-k) }
$$
...
$$
\boxed{ r_{xy}(l) \leftrightarrow X(k)Y^\ast (k) }
$$
...
$$
\boxed{ \text{IDFT}\{X\} = \frac1N \left[ \text{DFT}\{X^\ast \} \right]^\ast . }
$$
...
$$
\boxed{ X(N-k)=X^\ast (k) \quad\text{for real }x(n) }
$$
